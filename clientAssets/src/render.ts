...
private dropDownCart(){
    const button = $('.cart-toggle');
    button.on('click',(e:any)=>{
      const item = $(".dropdown-cart");
      item.fadeIn('slow');
    });
}
  private renderSummaryList(cart: CartItem) {
    let html = ``;
    html+=`<li class="dropdown-cart-header"><button class="close-cart-button">Close</button></li>`;
